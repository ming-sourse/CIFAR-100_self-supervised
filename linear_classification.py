...
model_full_classifier.backbone.load_state_dict(torch.load('simclr_resnet18_full.pth'), strict=False)
model_full_classifier = model_full_classifier.cuda()

criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model_full_classifier.fc.parameters(), lr=1e-3)

writer = SummaryWriter('runs/linear_full')
num_epochs = 50
for epoch in range(num_epochs):
    train(model_full_classifier, train_loader, criterion, optimizer, epoch, writer)
    test(model_full_classifier, test_loader, criterion, epoch, writer)
writer.close()